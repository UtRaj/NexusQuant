...
class SimulationRun(SQLModel, table=True):
    ...
    config_snapshot: Dict[str, Any] = Field(sa_column=Column(JSONB))
    status: str = "RUNNING"
...
class LLMAdvice(SQLModel, table=True):
    ...
    confidence: float
    rationale: str
    raw_response: Dict[str, Any] = Field(sa_column=Column(JSONB))
...
class PortfolioState(SQLModel, table=True):
    ...
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    balance: float
    holdings: Dict[str, float] = Field(sa_column=Column(JSONB))
    total_equity: float
